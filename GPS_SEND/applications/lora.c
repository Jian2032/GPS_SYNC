...
{
	HAL_UART_Transmit_DMA(&huart2,str,length);
}
...
{
	/* 确定起始标志位 */
	if(loraMaster.receiveData[0] == 0x03 && loraMaster.receiveData[1] == 0xfc)
	{
		/* 获取接收到数据的最后两位 */
		uint8_t last_1 = loraMaster.receiveData[30];
		uint8_t last   = loraMaster.receiveData[31];
		/* CRC校验 */
		Append_CRC16_Check_Sum(loraMaster.receiveData, 32);
		
		/* 如果满足要求进行解算 */
		if(last_1 == loraMaster.receiveData[30] && last == loraMaster.receiveData[31])
		{
		}
	}
}

// 给下位机监视器发送当前车的状态，以20hz频率
void Lora_TransmitCarData(void)
{
	Append_CRC16_Check_Sum(gps_send.bytes,sizeof(gps_send.bytes));
	Lora_Transmit_Data(gps_send.bytes,sizeof(gps_send.bytes));
}
